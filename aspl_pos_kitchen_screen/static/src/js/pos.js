odoo.define('aspl_pos_kitchen_screen.pos',function (require) {
	...
	chrome.Chrome.include({
		...
		_onNotification: function(notifications){
			var self = this;
			console.log("_onNotification CALLq", notifications);

			for (var notif of notifications) {
				console.log(notif, "notif......")
				if(notif[1].screen_display_data){
					console.log("screen_display_data::", notif[1].screen_display_data);
					console.log("screen_display_data.new_order::", notif[1].screen_display_data.new_order);
					console.log("screen_display_data.orders::", notif[1].screen_display_data.orders);
	...
	models.PosModel = models.PosModel.extend({
		mirror_kitchen_orders:function(new_order){
			console.log(new_order, "mirror kitchen orders!")
            rpc.query({
                model: 'pos.order',
	    ...
	    button_click : function() {
            ...
            if (orderLines.length === 0) {
                ...
                self.pos.push_order(selectedOrder).then(function(){
                    ...
                    self.pos.gui.screen_instances.products.order_widget.update_count_booked_orders()
                     console.log(">>>>>>>>>>>>ddddddddddddddddddddddddddddddddd>",selectedOrder.is_delivery)                    
if(selectedOrder.is_delivery){
                        selectedOrder.initialize_validation_date();
                        selectedOrder.finalized = true;
    ...
    models.Order = models.Order.extend({
        ...
        export_as_JSON: function() {
			...
			console.log(submitted_order, "inside submission order kitchen screen")
            var new_val = {
    ...
    var kitchenScreenWidget = screens.ScreenWidget.extend({
		...
		show: function() {
			...
			var self = this;
			this.categ_id = 0;
			self.call('bus_service', 'onNotification', self, self.chrome._onNotification);
			self.call('bus_service', 'startPolling');
			this.renderElement();
			if(self.pos.user.kitchen_screen_user === 'cook'){
		...
		events:{
			...
			'click .kitchen_screen_customer_feedbacks':'_onClickCustomerFeedbacks',
...
//        	self.pos.mirror_kitchen_orders();
		...
		_onClickCustomerFeedbacks: function(event){
		    var self = this;
		    var order_id = Number($(event.currentTarget).data('order-id'));
		    if(order_id){
		        rpc.query({
		            ...
		            method: 'search_read',
		            domain:[['pos_order_id', '=', parseInt(order_id)]]
		        }).then(function(result){
		            result = result.length > 0 && _.sortBy(result, 'id').reverse() || false;
                    self.gui.show_popup('PopupKitchenScreenComplaintFeedbackOrder', {
                        title: 'Complain / Feedback',
                        customer_feedback: result,
                    });
		        });
		    }
		},
    ...
    var PopupKitchenScreenComplaintFeedbackOrder = PopupWidget.extend({
        template: 'PopupKitchenScreenComplaintFeedbackOrder',
    });
    gui.define_popup({
        name: 'PopupKitchenScreenComplaintFeedbackOrder',
        widget: PopupKitchenScreenComplaintFeedbackOrder
    });
    ...
    return {
        SendOrderToKitchenButton: SendOrderToKitchenButton
    }
});
