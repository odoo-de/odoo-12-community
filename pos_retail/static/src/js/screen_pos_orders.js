...
odoo.define('pos_retail.screen_pos_orders', function (require) {
    ...
    var popup_register_payment = PopupWidget.extend({
        ...
        show: function (options) {
            ...
            console.log("Total Amount:", options.pos_order.amount_total);
            console.log("Amount Paid:", options.pos_order.amount_paid);
            console.log("Rounding Precision:", this.pos.currency.rounding);
    ...
    var button_go_pos_orders_screen = screens.ActionButtonWidget.extend({ // pos orders management
        ...
        button_click: function () {
            ...
            this.gui.show_screen('pos_orders_screen', {booked_order: false});
        }
    });
    ...
    var pos_orders_screen = screens.ScreenWidget.extend({
        ...
        get_booked_order: function(orders){
            console.log(orders, "ORDERS")
            console.log("get_booked_order CALLL")
            var show_booked_order = this.gui.get_current_screen_param('booked_order');
            console.log("show_booked_order", show_booked_order)
            if(show_booked_order){
                orders = _.sortBy(_.filter(orders, function(order){
                    return order.delivery_address_id && order.state == "draft";
                }), 'id').reverse();
            }
        ...
        render_pos_order_list: function (orders) {
            console.log("render_pos_order_list CALLLLLLLLLLLLLLLL", orders)
            var contents = this.$el[0].querySelector('.pos_order_list');
            orders = this.get_booked_order(orders);
            contents.innerHTML = "";
        ...
        display_pos_order_detail: function (order) {
            ...
            var customer_feedback = this.pos.db.get_feedback_by_order_id(order['id']);
            var line_contents = this.$('.customer_feedback_details');
            line_contents.empty();
            line_contents.append($(qweb.render('customer_feedback_lines', {widget: this, customer_feedback: customer_feedback || false})));
