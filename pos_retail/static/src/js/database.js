...
odoo.define('pos_retail.database', function (require) {
    ...
    db.include({
        ...
        get_pos_orders: function (max_count) {
            console.log('get_pos_orders function called with max_count:', max_count);
            
            var orders = [];
            var max = 0;
        
            // Log the current state of order_by_id
            console.log('Current order_by_id:', this.order_by_id);
        
            for (var order_id in this.order_by_id) {
                var order_by_id = this.order_by_id[order_id];
                
                // Log each order being processed
                console.log('Processing order:', order_by_id);
        
                this.get_delivery_address(order_by_id);
                orders.push(order_by_id);
        ...
        get_feedback_by_order_id: function(order_id){
            var customer_feedback = []
            rpc.query({
                model: 'customer.feedback',
                method: 'search_read',
                domain: [['pos_order_id', '=', order_id]],
            }, {async: false}).then(function (feedbacks) {
                feedbacks = feedbacks.length > 0 && _.sortBy(feedbacks, 'id').reverse() || false;
                customer_feedback = feedbacks;
            });
            return customer_feedback
        },
        get_delivery_address: function(order){
            delivery_address = false;
            if(order.delivery_address_id && order.delivery_address_id.length > 0){
                var partner = this.get_partner_by_id(order.delivery_address_id[0]);
                if(partner && partner.contact_address){
        ...
        save_pos_orders: function (orders) { // stores pos orders
            ...
            console.log("self> > > >  JAY >>>>>>", self)
            console.log("pos", self.pos)
            var branch_id = false;
            var pos_orders_filter_by_branch = false;
            if(self.posmodel && self.posmodel.config){
                branch_id = self.posmodel.config.pos_branch_id;
                pos_orders_filter_by_branch = self.posmodel.config.pos_orders_filter_by_branch;
            } else if(self.pos && self.pos.config){
                branch_id = self.pos.config.pos_branch_id;
                pos_orders_filter_by_branch = self.pos.config.pos_orders_filter_by_branch;
            }
