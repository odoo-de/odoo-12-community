...
//            }
    ...
    screens.define_action_button({
        ...
        'condition': function () {
            return this.pos.config.booking_orders && false;
        }
    });
    ...
    screens.define_action_button({
        ...
        'condition': function () {
            return this.pos.config.delivery_orders == true && false;
        }
    });
    ...
    screens.OrderWidget.include({
        init: function (parent, options) {
            ...
            this.pos.bind('new:pos_order', function () {
                self.update_count_booked_orders();
            }, this);
...
//            }
//        },
        update_count_booked_orders: function () { 
            var $booked_orders = $('.booked_orders');
            if ($booked_orders.length) {
                // Log to see if this function is called
                console.log('update_count_booked_orders function called');

                var pos_orders = this.pos.db.get_pos_orders(1000) || [];
                console.log('Fetched POS Orders:', pos_orders); // Log fetched orders

                var booking_orders = _.filter(pos_orders, function(order) {
                    return order.delivery_address_id && order.state === "draft";
                });

                // Log the filtered booking orders
                console.log('Filtered Booking Orders:', booking_orders);
                console.log('Booking Orders Count:', booking_orders.length);

                $booked_orders.text(booking_orders.length);
            } else {
                console.warn('Element .booked_orders not found');
            }
        },
